...
def lambda_handler(event, context):
    ...
    try:
        retention_policies = get_context_policies(POLICIES_LOCATION, policy_type="retention")
        logger.info("Context retention policies loaded successfully!")
    except Exception as e:
        logger.error("The following exception occured while loading the context policies: " + str(e))
    ...
    for record in event['Records']:
        ...
        try:
            ...
            logger.error("The following exception occured while processing the event: " + str(e))
        ...
        if context_obj.validate_sensor_reading_ttl(retention_policies):
            ...
            try:
                ...
                logger.error("The following exception occured when attempting to build context: " + str(e))
                logger.info("Skipping this event...")
                continue
            
            logger.info("Context built successfully!")
            logger.info("Storing the context instance in the Context table in DynamoDB...")

            try:
                data_store.put([context_instance], 'Context')
            except:
                logger.error("The following exception occured when attempting to store the context instance: " + str(e))
                logger.info("Skipping this event...")
                continue
